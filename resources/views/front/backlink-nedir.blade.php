...
<section class="py-12 pb-28">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="bg-zinc-900/50 border border-zinc-800/60 rounded-2xl p-8">
            <h2 class="text-xl font-bold text-white mb-3" style="font-family:'Bricolage Grotesque',sans-serif">{{ __('backlink.baclink1') }}</h2>
            <p class="text-zinc-400 text-sm leading-relaxed mb-3">{{ __('backlink.baclink2') }}</p>
            <p class="text-zinc-400 text-sm leading-relaxed">{{ __('backlink.baclink3') }}</p>
        </div>

        {{-- How to get backlinks --}}
        <div class="bg-zinc-900/40 border border-zinc-800/50 rounded-2xl p-8">
            <h2 class="text-xl font-bold text-white mb-3" style="font-family:'Bricolage Grotesque',sans-serif">{{ __('backlink.baclink4') }}</h2>
            <p class="text-zinc-400 text-sm leading-relaxed mb-3">{{ __('backlink.baclink5') }}</p>
            <p class="text-zinc-400 text-sm leading-relaxed mb-5">{{ __('backlink.baclink6') }}</p>
            <ul class="space-y-3">
                @foreach([
                    __('backlink.baclink7'), __('backlink.baclink8'), __('backlink.baclink9'),
                    __('backlink.baclink10'), __('backlink.baclink11'),
                ] as $item)
