...
<div class="flex items-center justify-between mb-7">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Şirkət Düzəliş</h1>
...
<form id="cmpForm" action="{{ route('company.update', $company->id) }}" method="POST" enctype="multipart/form-data">
@csrf @method('PUT')
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="group_id" class="admin-select">
                        ...
                        <option value="{{ $group->id }}" {{ $company->group_id==$group->id ? 'selected' : '' }}>{{ $group->name_az }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="admin-label">Şirkət adı *</label>
                    <input type="text" name="name" value="{{ old('name', $company->name) }}" class="admin-input @error('name') border-red-400 @enderror">
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="alt" value="{{ old('alt', $company->alt) }}" class="admin-input">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="p-6">
                @foreach(['az'=>$company->about_az,'en'=>$company->about_en,'ru'=>$company->about_ru] as $l=>$val)
                <div x-show="lang==='{{ $l }}'">
                    <textarea name="about_{{ $l }}" id="about_{{ $l }}" rows="8"
                              class="admin-input resize-none @error('about_'.$l) border-red-400 @enderror">{{ old('about_'.$l, $val) }}</textarea>
    ...
    <div class="space-y-5">
        <div class="form-card form-card-body">
            ...
            @if($company->src)
            <div class="mb-3 bg-gray-50 border border-gray-100 rounded-xl p-3 flex items-center justify-center h-28">
                <img src="{{ asset('storage/'.$company->src) }}" class="max-h-full w-auto object-contain" alt="">
            </div>
            @endif
            <label class="upload-zone h-28 group">
                ...
                <div id="logoPh" class="flex flex-col items-center gap-2 text-gray-400 group-hover:text-indigo-500 transition-colors p-4">
                    <i class="fa fa-arrow-up-from-bracket text-2xl"></i>
                    <span class="text-xs font-semibold">{{ $company->src ? 'Yenilə' : 'Loqo seç' }}</span>
                </div>
                <img id="logoPrev" src="#" class="hidden absolute inset-0 w-full h-full object-contain p-2">
            </label>
        </div>
        ...
        <div class="form-card form-card-body">
            ...
            <div class="text-xs text-gray-500 space-y-2">
                <div class="flex justify-between"><span>ID</span><span class="font-mono font-semibold text-gray-700">#{{ $company->id }}</span></div>
                <div class="flex justify-between"><span>Qrup</span><span class="text-gray-700">{{ $company->group->name_az ?? '—' }}</span></div>
            </div>
        </div>
    </div>
</div>
