...
@section('title', 'Kateqoriyanı Düzəlt')
...
<div class="flex items-center justify-between mb-7">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Kateqoriyanı Düzəlt</h1>
            <p class="text-xs text-gray-400 mt-0.5">{{ $category->name_az }}</p>
        </div>
    </div>
    <div class="flex items-center gap-3">
...
<div class="max-w-xl">
    <form id="catForm" action="/admin/note-categories/{{ $category->id }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form-card form-card-body space-y-4">
            <div>
                ...
                <input type="text" name="name_az" id="name_az" value="{{ $category->name_az }}" class="admin-input" placeholder="Kateqoriya adı (AZ)">
            ...
            <div>
                ...
                <input type="text" name="name_en" id="name_en" value="{{ $category->name_en }}" class="admin-input" placeholder="Category name (EN)">
...
function submitForm() {
    ...
    $.ajax({
        type: 'POST', url: '/admin/note-categories/{{ $category->id }}',
