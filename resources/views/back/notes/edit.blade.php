...
<div class="flex items-center justify-between mb-7">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Qeydi Düzəlt</h1>
            <p class="text-xs text-gray-400 mt-0.5 line-clamp-1">{{ $note->title_az ?: $note->title_en }}</p>
        </div>
    </div>
    <div class="flex items-center gap-3">
...
<form id="noteForm" action="/admin/notes/{{ $note->id }}" method="POST">
@csrf
@method('PUT')
            ...
            <div class="p-6 space-y-4">
                <div x-show="lang==='az'" class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title_az" id="title_az" value="{{ $note->title_az }}" class="admin-input" placeholder="Qeydin başlığı (AZ)">
                    ...
                    <div>
                        ...
                        <textarea name="body_az" id="body_az" rows="16" class="admin-input resize-none">{{ $note->body_az }}</textarea>
                ...
                <div x-show="lang==='en'" class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title_en" id="title_en" value="{{ $note->title_en }}" class="admin-input" placeholder="Note title (EN)">
                    ...
                    <div>
                        ...
                        <textarea name="body_en" id="body_en" rows="16" class="admin-input resize-none">{{ $note->body_en }}</textarea>
    ...
    <div class="space-y-5">
        <div class="form-card form-card-body">
            ...
            <select name="note_category_id" class="admin-input">
                ...
                <option value="{{ $cat->id }}" {{ $note->note_category_id == $cat->id ? 'selected' : '' }}>{{ $cat->name_az }}</option>
                @endforeach
            </select>
        </div>

        <div class="form-card form-card-body text-xs text-gray-400 space-y-1">
            <div>Yaradıldı: {{ $note->created_at?->format('d.m.Y H:i') }}</div>
            <div>Yeniləndi: {{ $note->updated_at?->format('d.m.Y H:i') }}</div>
        </div>
    </div>
</div>
</form>
...
function submitNote() {
    ...
    $.ajax({
        type: 'POST', url: '/admin/notes/{{ $note->id }}',
