    ...
    <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2">
            ...
            <button id="aiBlogBtn" type="button" onclick="generateWithAI()"
                ...
                <i class="fa fa-wand-magic-sparkles"></i> AI ilə yaz
            </button>
        </div>
                    ...
                    <div class="border-t border-gray-100 pt-4">
                        ...
                        <label class="relative flex flex-col items-center justify-center h-40 rounded-xl border-2 border-dashed border-gray-200 hover:border-indigo-400 cursor-pointer overflow-hidden group transition-colors">
                            <input type="file" name="{{ $photoField }}" id="{{ $photoField }}" accept="image/*"
                                   ...
                                   onchange="previewImg(this,'covPrev_{{ $l }}','covPh_{{ $l }}'); clearAiPhoto('{{ $l }}')">
...
async function generateWithAI() {
    ...
    try {
        ...
        if (result.image_preview && result.image_files) {
            ...
            toastr.success('Blog və şəkil AI tərəfindən hazırlandı!');
        } else {
            toastr.success('Blog AI tərəfindən yazıldı! Şəkil əlavə edin.');
        }
    } catch (e) {
        toastr.error('Şəbəkə xətası baş verdi');
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="fa fa-wand-magic-sparkles"></i> AI ilə yaz';
    }
}
